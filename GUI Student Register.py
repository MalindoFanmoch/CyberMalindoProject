...
# Check if Password is Not Empty So it will Display Congrats Message
        if(password.get() != "" and Name != "" and (YY < current_year - 4) and (Course != "Select Course" or Branch != "Select Branch") and ((Gender == 1) or (Gender == 2)) and Phone != "" and Email != ""):
	msg.showinfo("Congratulation", "You Registered Successfully!")
     
# Create a Function to Reset All the Entry 
def reset():
    Entry(root, validate = "key", validatecommand = valid_num, font = ("lucida 22 bold"), bd = 2).place(x = 380, y = 385, width = 300)
    name.set("")
    date.set("1")
    month.set("January")
    year.set("2021")
...
Button(root, text = "Reset", font = ("lucida 22 bold"), command = reset, bg = "yellow3", activebackground = "darksalmon", bd =5).place(x = 170, y = 545)
 
# Button for Cancel Entry Data
Button(root, text = "Cancel", font = ("lucida 22 bold"), command = quit, bg = "indianred1", activebackground = "darksalmon", bd = 5).place(x = 340, y = 545)
 
# Button for Submit Entry Data
Button(root, text = "Submit", font = ("lucida 22 bold"), command = submit, bg = "lawngreen", activebackground = "chartreuse3", bd = 5).place(x = 520, y = 545)
